...
```bash
STACK_NAME=stag-domicile-patrickblack-dev sh ./scripts/deploy.sh
```
...
```bash
docker node update --label-add domicile-patrickblack-dev.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stag-domicile-patrickblack-dev.app-db-data=true cat.example.com
```
...
* Set these environment variables:
  * `DOMAIN=domicile.patrickblack.dev`
  * `TRAEFIK_TAG=domicile.patrickblack.dev`
  * `STACK_NAME=domicile-patrickblack-dev`
  * `TAG=prod`
...
Frontend: https://domicile.patrickblack.dev

Backend: https://domicile.patrickblack.dev/api/

Automatic Interactive Docs (Swagger UI): https://domicile.patrickblack.dev/docs
