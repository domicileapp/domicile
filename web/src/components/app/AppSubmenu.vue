...
function onmenuItemClick(
  ...
  item: { disabled: any; to: any; url: any; command: (arg0: { originalEvent: any; item: any }) => void },
  index: any
) {
  if (item.disabled) {
    ...
    <template v-for="(item, i) of items">
      ...
      >
        <template v-if="root">
          ...
          <AppSubmenu :items="visible(item) && item.items" @menu-item-click="$emit('menuItemClick', $event)" />
        </template>
        <template v-else>
          <router-link
            v-if="item.to"
