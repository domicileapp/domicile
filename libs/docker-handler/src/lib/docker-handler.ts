...
export class DockerHandler {
   ...
   * `./apps/docker/secrets`; override default with
   ...
   * @example
   * catch (err) {
   * 	printReadError(err, "SECRET", "./apps/docker/secrets")
   * }
   *
...
export interface DockerHandlerOptions {
   ...
   *  for development, from project root. Default: `./apps/docker/secrets/`. */
...
export interface DockerGetSecretOptions {
   ...
   * secrets are located. Default: `./apps/docker/secrets`; override default
